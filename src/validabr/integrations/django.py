"""Django integration for validabr.

This module requires Django to be installed:
    pip install django>=4.0

Model Fields:
    from validabr.integrations.django import CPFField

    class Person(models.Model):
        cpf = CPFField()

Form Fields:
    from validabr.integrations.django import CPFFormField
...
try:
    ...
    raise ImportError(
        "Django is required for validabr.integrations.django. "
        "Install it with: pip install validabr[django]"
    ) from exc
...
class CPFFormField(django_forms.CharField):
    ...
    def clean(self, value: Any) -> str:
        ...
        if not validabr.is_valid_cpf(digits):
            from django.core.exceptions import ValidationError

            raise ValidationError("Invalid CPF.")
        return validabr.format_cpf(digits)
...
class CNPJFormField(django_forms.CharField):
    ...
    def clean(self, value: Any) -> str:
        ...
        if not validabr.is_valid_cnpj(digits):
            from django.core.exceptions import ValidationError

            raise ValidationError("Invalid CNPJ.")
        return validabr.format_cnpj(digits)
...
class CNJFormField(django_forms.CharField):
    ...
    def clean(self, value: Any) -> str:
        ...
        if not validabr.is_valid_cnj(digits):
            from django.core.exceptions import ValidationError

            raise ValidationError("Invalid CNJ.")
        return validabr.format_cnj(digits)
...
class RenavamFormField(django_forms.CharField):
    ...
    def clean(self, value: Any) -> str:
        ...
        if not validabr.is_valid_renavam(digits):
            from django.core.exceptions import ValidationError

            raise ValidationError("Invalid RENAVAM.")
        return validabr.format_renavam(digits)
...
class TituloEleitorFormField(django_forms.CharField):
    ...
    def clean(self, value: Any) -> str:
        ...
        if not validabr.is_valid_titulo_eleitor(digits):
            from django.core.exceptions import ValidationError

            raise ValidationError("Invalid Título de Eleitor.")
        return validabr.format_titulo_eleitor(digits)
...
class IEFormField(django_forms.CharField):
    ...
    def clean(self, value: Any) -> str:
        ...
        if not isinstance(value, str):
            ...
            raise ValidationError("Invalid input type.")
        digits = re.sub(r"\D", "", value)
        if not validabr.is_valid_ie(digits, self._state):
            from django.core.exceptions import ValidationError

            raise ValidationError("Invalid Inscrição Estadual.")
        return validabr.format_ie(digits, self._state)
