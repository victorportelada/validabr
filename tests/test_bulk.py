...
import validabr
from validabr.bulk import generate_list, validate_docs, validate_list


class TestValidateList:
    def test_all_valid_cpfs(self) -> None:
        valid = validabr.generate_cpf()
    ...
    def test_result_length_matches_input(self) -> None:
        cpfs = [validabr.generate_cpf() for _ in range(5)]
...
class TestGenerateList:
    ...
    def test_all_generated_are_valid(self) -> None:
        ...
        assert all(validabr.is_valid_cpf(v) for v in result)
    ...
    def test_generates_cnpj(self) -> None:
        ...
        assert all(validabr.is_valid_cnpj(v) for v in result)

    def test_generates_renavam(self) -> None:
        result = generate_list("renavam", 3)
        assert all(validabr.is_valid_renavam(v) for v in result)

    def test_generates_titulo_eleitor(self) -> None:
        result = generate_list("titulo_eleitor", 3)
        assert all(validabr.is_valid_titulo_eleitor(v) for v in result)
    ...
    def test_skips_duplicates(self) -> None:
        ...
        cpf_a = validabr.generate_cpf()
        cpf_b = validabr.generate_cpf()
        side_effects = [cpf_a, cpf_a, cpf_b]  # first duplicate triggers skip branch
        with patch("validabr.bulk._GENERATORS", {"cpf": iter(side_effects).__next__}):
...
class TestValidateDocs:
    ...
    def test_mixed_types(self) -> None:
        cpf = validabr.generate_cpf()
        cnpj = validabr.generate_cnpj()
    ...
    def test_result_length_matches_input(self) -> None:
        pairs = [("cpf", validabr.generate_cpf()) for _ in range(5)]
...
class TestBulkExportedFromBrdocs:
    def test_validate_list_importable_from_brdocs(self) -> None:
        assert hasattr(validabr, "validate_list")

    def test_generate_list_importable_from_brdocs(self) -> None:
        assert hasattr(validabr, "generate_list")

    def test_validate_docs_importable_from_brdocs(self) -> None:
        assert hasattr(validabr, "validate_docs")
