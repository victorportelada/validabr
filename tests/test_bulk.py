...
import brdocs
from brdocs.bulk import generate_list, validate_docs, validate_list


class TestValidateList:
    def test_all_valid_cpfs(self) -> None:
        valid = brdocs.generate_cpf()
    ...
    def test_result_length_matches_input(self) -> None:
        cpfs = [brdocs.generate_cpf() for _ in range(5)]
...
class TestGenerateList:
    ...
    def test_all_generated_are_valid(self) -> None:
        ...
        assert all(brdocs.is_valid_cpf(v) for v in result)
    ...
    def test_generates_cnpj(self) -> None:
        ...
        assert all(brdocs.is_valid_cnpj(v) for v in result)

    def test_generates_renavam(self) -> None:
        result = generate_list("renavam", 3)
        assert all(brdocs.is_valid_renavam(v) for v in result)

    def test_generates_titulo_eleitor(self) -> None:
        result = generate_list("titulo_eleitor", 3)
        assert all(brdocs.is_valid_titulo_eleitor(v) for v in result)
    ...
    def test_skips_duplicates(self) -> None:
        ...
        cpf_a = brdocs.generate_cpf()
        cpf_b = brdocs.generate_cpf()
        side_effects = [cpf_a, cpf_a, cpf_b]  # first duplicate triggers skip branch
        with patch("brdocs.bulk._GENERATORS", {"cpf": iter(side_effects).__next__}):
...
class TestValidateDocs:
    ...
    def test_mixed_types(self) -> None:
        cpf = brdocs.generate_cpf()
        cnpj = brdocs.generate_cnpj()
    ...
    def test_result_length_matches_input(self) -> None:
        pairs = [("cpf", brdocs.generate_cpf()) for _ in range(5)]
