"""Tests for validabr Polars DataFrame extension integration."""
...
import validabr.integrations.polars  # noqa: E402 - registers extension


class TestIsValidExpr:
    def test_is_valid_cpf_true(self) -> None:
        df = polars.DataFrame({"cpf": ["529.982.247-25"]})
        result = df.select(polars.col("cpf").validabr.is_valid_cpf())
    ...
    def test_is_valid_cnpj_true(self) -> None:
        ...
        result = df.select(polars.col("cnpj").validabr.is_valid_cnpj())
        assert result[0, 0] is True
    ...
    def test_is_valid_ie_true(self) -> None:
        ie = validabr.generate_ie("SP")
        df = polars.DataFrame({"ie": [ie]})
        result = df.select(polars.col("ie").validabr.is_valid_ie("SP"))
        assert result[0, 0] is True
    ...
    def test_is_valid_cep_true(self) -> None:
        ...
        result = df.select(polars.col("cep").validabr.is_valid_cep())
        assert result[0, 0] is True
    ...
    def test_is_valid_cnh_true(self) -> None:
        cnh = validabr.generate_cnh()
        df = polars.DataFrame({"cnh": [cnh]})
        result = df.select(polars.col("cnh").validabr.is_valid_cnh())
        assert result[0, 0] is True
    ...
    def test_is_valid_pis_true(self) -> None:
        pis = validabr.generate_pis()
        df = polars.DataFrame({"pis": [pis]})
        result = df.select(polars.col("pis").validabr.is_valid_pis())
        assert result[0, 0] is True
    ...
    def test_is_valid_renavam_true(self) -> None:
        ...
        result = df.select(polars.col("renavam").validabr.is_valid_renavam())
        assert result[0, 0] is True
    ...
    def test_is_valid_titulo_eleitor_true(self) -> None:
        ...
        result = df.select(polars.col("titulo").validabr.is_valid_titulo_eleitor())
        assert result[0, 0] is True
    ...
    def test_is_valid_cnj_true(self) -> None:
        ...
        result = df.select(polars.col("cnj").validabr.is_valid_cnj())
        assert result[0, 0] is True
    ...
    def test_is_valid_nfe_false(self) -> None:
        ...
        result = df.select(polars.col("nfe").validabr.is_valid_nfe())
        assert result[0, 0] is False

    def test_classify_pix_cpf(self) -> None:
        df = polars.DataFrame({"pix": ["529.982.247-25"]})
        result = df.select(polars.col("pix").validabr.classify_pix())
...
class TestFormatExpr:
    def test_format_cpf(self) -> None:
        ...
        result = df.select(polars.col("cpf").validabr.format_cpf())
    ...
    def test_format_cnpj(self) -> None:
        ...
        result = df.select(polars.col("cnpj").validabr.format_cnpj())
    ...
    def test_format_titulo_eleitor(self) -> None:
        raw = validabr.generate_titulo_eleitor()
        df = polars.DataFrame({"titulo": [raw]})
        result = df.select(polars.col("titulo").validabr.format_titulo_eleitor())
    ...
    def test_format_cnj(self) -> None:
        raw = validabr.generate_cnj()
        df = polars.DataFrame({"cnj": [raw]})
        result = df.select(polars.col("cnj").validabr.format_cnj())
        assert "-" in result[0, 0]
        assert "." in result[0, 0]
    ...
    def test_format_nfe(self) -> None:
        ...
        result = df.select(polars.col("nfe").validabr.format_nfe())
...
class TestMaskExpr:
    def test_mask_cpf(self) -> None:
        df = polars.DataFrame({"cpf": ["529.982.247-25"]})
        result = df.select(polars.col("cpf").validabr.mask_cpf())
    ...
    def test_mask_cnpj(self) -> None:
        df = polars.DataFrame({"cnpj": ["11.222.333/0001-81"]})
        result = df.select(polars.col("cnpj").validabr.mask_cnpj())
...
class TestChainedExpressions:
    def test_chain_is_valid_and_format(self) -> None:
        ...
        result = df.select(
            polars.col("cpf").validabr.is_valid_cpf().alias("valid"),
        )
    ...
    def test_filter_valid_cpf(self) -> None:
        ...
        valid = df.filter(polars.col("cpf").validabr.is_valid_cpf())
        assert len(valid) == 1
