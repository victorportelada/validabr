"""Tests for brdocs Django integration."""
...
class TestCPFModelField:
    ...
    def test_round_trip(self) -> None:
        ...
        p = Person(cpf=raw)
        p.full_clean()
        assert brdocs.is_valid_cpf(p.cpf)
...
class TestCNJModelField:
    def test_accepts_valid_cnj(self) -> None:
        ...
        p = Process(cnj=raw)
        p.full_clean()
        assert brdocs.is_valid_cnj(p.cnj)
...
class TestRenavamModelField:
    def test_accepts_valid_renavam(self) -> None:
        ...
        v.full_clean()
        assert brdocs.is_valid_renavam(v.renavam)
...
class TestTituloEleitorModelField:
    def test_accepts_valid_titulo(self) -> None:
        ...
        v = Voter(titulo=raw)
        v.full_clean()
        assert brdocs.is_valid_titulo_eleitor(v.titulo)
...
class TestIEModelField:
    def test_accepts_valid_ie_with_state(self) -> None:
        ...
        e.full_clean()
        assert brdocs.is_valid_ie(e.ie, "SP")
    ...
    def test_different_states(self) -> None:
        ...
        raw = brdocs.generate_ie("MG")
        e = Establishment(ie=raw)
        e.full_clean()
        assert brdocs.is_valid_ie(e.ie, "MG")
...
class TestIEFormField:
    def test_accepts_valid_ie_with_state(self) -> None:
        ...
        assert f.is_valid(), f.errors
        assert brdocs.is_valid_ie(f.cleaned_data["ie"], "SP")
